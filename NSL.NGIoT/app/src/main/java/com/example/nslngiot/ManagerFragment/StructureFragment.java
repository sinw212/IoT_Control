...
public class StructureFragment extends Fragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        StructureImage = getView().findViewById(R.id.pho_manager_structure);
        gallery = getView().findViewById(R.id.btn_picture);
        upload = getView().findViewById(R.id.btn_add);

        structuerFile_Upload_Request(2);
            ...
            public void onClick(View view) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            // 이미지 저장
                            structuerFile_Upload_Request(1);
                            Thread.sleep(1000);
                            // 이미지 조회
                            structuerFile_Upload_Request(2);
                        } catch (InterruptedException e) {
                            System.err.println("Manager StructureFramgent InterruptedException error");
                        }
                    }
                }).start();
            }
        ...
        if (requestCode == REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK){
                try {
                    ...
                    System.err.println("Manager StructureFramgent onActivityResult FileNotFoundException error");
                } finally {
                    if(inputStream != null){
                        try{
                            inputStream.close();
                        }catch (IOException e) {
                            System.err.println("Manager StructureFramgent onActivityResult IOException error");
                        }
                    }
                    if(cursor != null)
                        cursor.close();
    ...
    //이미지 전송 및 조회
    private void structuerFile_Upload_Request(final int menu) {
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                switch (menu) {
                    case 1: // 이미지 전송
                        ...
                        params.put("type",AES.aesEncryption("strUpload".toCharArray(),decryptAESkey_upload));
        ...
        try{
            ...
            if(byteArrayOutputStream != null){
                try{
                    ...
                    System.err.println("Manager StructureFramgent BitmapToString IOException error");
                }
            }
        }
        return temp;
